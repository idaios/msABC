...
int main(int argc, char** argv){
  ...
  */
  ...
  void constructfragpars(int n, int size, double minrec, double maxrec, double minmu, double maxmu, int length);
...
/*       fprintf(stderr, "\n"); */
    ...
    */
      ...
      if(count <= 1)
	{
      ...
      /* if fragfile is given then read the data */
      if(fragfile != NULL && count <=1){
	...
	*/
      ...
      else if(fragfile == NULL)
	{
	  ...
	  if(minmu == maxmu && maxmu == 0.0)
	    {
	      maxmu = pars.mp.theta / pars.cp.nsites / effectiveN / 4.0;
	      minmu = maxmu;
	    }

	  if(minrec == maxrec && maxrec == 0.0)
	    {
	      maxrec = pars.cp.r / pars.cp.nsites / effectiveN / 4.0;
	      minrec = maxrec;
	    }
	  
	  //fprintf(stderr, "%e\t%e\t%e\t%e\n", minrec, maxrec, minmu, maxmu);

	  

	  constructfragpars(nofragments, pars.cp.nsam, minrec, maxrec, minmu, maxmu, fraglength);
	...
	*/
	...
	if( (obin == NULL) && tempdatafile!=NULL ){ 
	  ...
	  printf(tempdatafile, "\t%d\t%d\t%e\t%e\t%d", count, i, pars.mp.theta, pars.cp.r, flength[i]);
	  fprintf(tempdatafile, "\n");
	...
	if(count == 1){
	  for(i=3; i<argc; i++){
		...
		if(printall || param_distr[i] != '-')
	      ...
	      if(!strcmp(argv[i], "--minrec"))
		if(printall || param_distr[i] != '-')
		  fprintf(pf, "p_minrec\t");
	      
	      if(!strcmp(argv[i], "--maxrec"))
		if(printall || param_distr[i] != '-')
		  fprintf(pf, "p_maxrec\t");
	      
	      if(!strcmp(argv[i], "--minmu"))
		if(printall || param_distr[i] != '-')
		  fprintf(pf, "p_minmu\t");
	      
	      if(!strcmp(argv[i], "--maxmu"))
		if(printall || param_distr[i] != '-')
		  fprintf(pf, "p_maxmu\t");
...
getpars(int argc, char *argv[], int *phowmany )
{
  ...
  while( arg < argc ){
	...
	if(!strcmp( argv[arg], "--finp")){
	  fragfile = malloc(100*sizeof(char));
	  if(argv[arg+1][0] != '-')
	...
	if(!strcmp( argv[arg], "--fraglength"))
	  {
	    assert(fragfile == NULL);
	    fraglength = atoi(argv[++arg]);
	    continue;
	  }
	if(!strcmp( argv[arg], "--nfrag"))
	  {
	    assert(fragfile == NULL);
	...
	if(!strcmp( argv[arg], "--minmu" ))
	  {
		...
		minmu = get_param(arg - 1, argv[arg][1]);
		global_values[arg-1] = minmu;
	...
	if(!strcmp( argv[arg], "--maxmu"))
	  {
		...
		maxmu = get_param(arg - 1, argv[arg][1]);
		global_values[arg-1] = maxmu;
		arg = arg + distribution_arguments( param_distr[arg - 1] ) + 1;	    
	      }
	    else
	      {
	...
	if(!strcmp( argv[arg], "--minrec"))
	  {
	    ...
	    if( !count && (c = dist_argcheck(arg, argc, argv)) )
	      ...
	      {
		minrec = get_param(arg - 1, argv[arg][1]);
		global_values[arg-1] = minrec;
		arg = arg + distribution_arguments( param_distr[arg - 1] ) + 1;	    
	      }
	    else
	      {
		argcheck( arg, argc, argv);
		minrec = atof(  argv[arg] );
		global_values[arg-1] = minrec;
		arg++;
	      }
	    arg += c-1;
	...
	if(!strcmp( argv[arg], "--maxrec"))
	  {
	    ...
	    if( !count && (c = dist_argcheck(arg, argc, argv)) )
	      ...
	      {
		maxrec = get_param(arg - 1, argv[arg][1]);
		global_values[arg-1] = maxrec;
		arg = arg + distribution_arguments( param_distr[arg - 1] ) + 1;	    
	      }
	    else
	      {
		argcheck( arg, argc, argv);
		maxrec = atof(  argv[arg] );
		global_values[arg-1] = maxrec;
		arg++;
	      }
	    arg += c-1;
...
int getobservations(FILE* obin, char*** data){
  ...
  while( fscanf(obin, "%s", line) != EOF){
	...
	j++;
	assert(j < HARD_LIMIT); // && i < pars.cp.nsam);
      }

      
	      
      //printf("j: %d\ni: %d\npars.cp.nsam: %d\n", j, i, pars.cp.nsam);
      assert(i < pars.cp.nsam);
  ...
  for(i=0; i<n; ++i)
    {
      fsize[i] = size;
      // fname[i] = "frag";
      fmubp[i] = (rand()/((double)RAND_MAX + 1) * (maxmu - minmu)) + minmu;
      frecbp[i] = (rand()/((double)RAND_MAX + 1) * (maxrec - minrec)) + minrec;
      fpop[i] = 0;
      flength[i] = length;
